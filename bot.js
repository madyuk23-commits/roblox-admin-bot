...
app.get('/api/admins', (req, res) => { console.log(`📡 Запрос админов от Roblox`); res.json({ admins: robloxAdmins }); });
...
app.post('/api/roblox/update-balance', (req, res) => { const { userId, amount } = req.body; userBalances[userId] = (userBalances[userId] || 0) + amount; console.log(`💰 Баланс ${userId}: ${userBalances[userId]}`); res.json({ success: true }); });
...
const commandsData = [
    ...
    new SlashCommandBuilder().setName('bankgive').setDescription('Выдать монеты').addIntegerOption(opt => opt.setName('robloxid').setDescription('Roblox ID').setRequired(true)).addIntegerOption(opt => opt.setName('amount').setDescription('Количество').setRequired(true)),
    new SlashCommandBuilder().setName('bankremove').setDescription('Забрать монеты').addIntegerOption(opt => opt.setName('robloxid').setDescription('Roblox ID').setRequired(true)).addIntegerOption(opt => opt.setName('amount').setDescription('Количество').setRequired(true)),
    ...
    new SlashCommandBuilder().setName('xp_give').setDescription('Выдать XP').addUserOption(opt => opt.setName('user').setRequired(true)).addIntegerOption(opt => opt.setName('amount').setRequired(true)).addStringOption(opt => opt.setName('reason')),
    new SlashCommandBuilder().setName('xp_info').setDescription('Узнать XP').addUserOption(opt => opt.setName('user')),
    new SlashCommandBuilder().setName('gamerole').setDescription('Изменить ранг полиции').addIntegerOption(opt => opt.setName('robloxid').setDescription('Roblox ID').setRequired(true)).addStringOption(opt => opt.setName('rank').setDescription('Название ранга').setRequired(true))
];
...
client.once('ready', async () => {
    ...
    console.log(`🌐 API доступен по адресу: https://${process.env.RENDER_EXTERNAL_URL || 'localhost'}/api/admins`);
});

client.on('interactionCreate', async interaction => {
    if (!interaction.isCommand()) return;
    if (!ALLOWED_USERS.includes(interaction.user.id)) return interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('⛔ ДОСТУП ЗАПРЕЩЁН')], ephemeral: true });
    ...
    if (commandName === 'removeadmin') {
        ...
        interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('❌ АДМИНИСТРАТОР УДАЛЁН')], ephemeral: true });
    }
    ...
    if (commandName === 'finduser') {
        ...
        if (data.data?.length) { const user = data.data[0]; const isAdmin = robloxAdmins.includes(user.id); interaction.reply({ embeds: [new EmbedBuilder().setColor(isAdmin ? 0x00FF00 : 0xAAAAAA).setTitle('🔍 РЕЗУЛЬТАТ ПОИСКА').setDescription(`**Имя:** ${user.name}\n**ID:** ${user.id}\n**Статус:** ${isAdmin ? '✅ Администратор' : '❌ Не администратор'}\n**Ссылка:** https://www.roblox.com/users/${user.id}/profile`)], ephemeral: true }); }
        else { interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('❌ Не найдено').setDescription(`Пользователь ${username} не найден!`)], ephemeral: true }); }
    ...
    // БАНК
    if (commandName === 'bankgive') { userBalances[robloxId] = (userBalances[robloxId] || 0) + amount; const info = await getRobloxUserInfo(robloxId); interaction.reply({ embeds: [new EmbedBuilder().setColor(0x00FF00).setTitle('💰 МОНЕТЫ ВЫДАНЫ').setDescription(`**Игрок:** ${info?.name || robloxId}\n**Выдано:** ${amount} 💎\n**Новый баланс:** ${userBalances[robloxId]} 💎`)], ephemeral: true }); }
    if (commandName === 'bankremove') { userBalances[robloxId] = Math.max(0, (userBalances[robloxId] || 0) - amount); const info = await getRobloxUserInfo(robloxId); interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('💰 МОНЕТЫ ЗАБРАНЫ').setDescription(`**Игрок:** ${info?.name || robloxId}\n**Забрано:** ${amount} 💎\n**Новый баланс:** ${userBalances[robloxId]} 💎`)], ephemeral: true }); }
    if (commandName === 'bankinfo') { const balance = userBalances[robloxId] || 0; const info = await getRobloxUserInfo(robloxId); interaction.reply({ embeds: [new EmbedBuilder().setColor(0x00AAFF).setTitle('💰 БАЛАНС ИГРОКА').setDescription(`**Игрок:** ${info?.name || robloxId}\n**Баланс:** ${balance} 💎`)], ephemeral: true }); }

    // XP
    if (commandName === 'xp_give') { const targetUser = interaction.options.getUser('user'); const xpAmount = interaction.options.getInteger('amount'); const reason = interaction.options.getString('reason') || 'Без причины'; userXP[targetUser.id] = (userXP[targetUser.id] || 0) + xpAmount; await interaction.reply({ embeds: [new EmbedBuilder().setColor(0x00FF00).setTitle('✨ XP ВЫДАНЫ').setDescription(`**Кому:** ${targetUser}\n**Количество:** ${xpAmount} XP\n**Причина:** ${reason}`)], ephemeral: true }); await sendLog(new EmbedBuilder().setColor(0x00AAFF).setTitle('📝 ЛОГ ВЫДАЧИ XP').setDescription(`**Кому:** ${targetUser} (${targetUser.id})\n**Количество:** ${xpAmount} XP\n**Причина:** ${reason}\n**Выдал:** ${interaction.user.tag}`).setTimestamp()); }
    if (commandName === 'xp_info') { const targetUser = interaction.options.getUser('user') || interaction.user; const xp = userXP[targetUser.id] || 0; interaction.reply({ embeds: [new EmbedBuilder().setColor(0x00AAFF).setTitle('📊 ИНФОРМАЦИЯ О XP').setDescription(`**Пользователь:** ${targetUser}\n**Всего XP:** ${xp}`)], ephemeral: true }); }

    // РАНГИ
    if (commandName === 'gamerole') { const rankName = interaction.options.getString('rank'); if (!POLICE_RANKS.includes(rankName)) return interaction.reply({ embeds: [new EmbedBuilder().setColor(0xFF0000).setTitle('❌ Ошибка').setDescription(`Ранг "${rankName}" не найден!`)], ephemeral: true }); userRanks[robloxId] = rankName; const info = await getRobloxUserInfo(robloxId); await interaction.reply({ embeds: [new EmbedBuilder().setColor(0x00AAFF).setTitle('👮‍♂️ РАНГ ПОЛИЦИИ ИЗМЕНЁН').setDescription(`**Игрок:** ${info?.name || robloxId}\n**Новый ранг:** ${rankName}\n**Выдал:** ${interaction.user.tag}`)], ephemeral: true }); await sendLog(new EmbedBuilder().setColor(0x00AAFF).setTitle('📝 ЛОГ ВЫДАЧИ РАНГА').setDescription(`**Игрок:** ${info?.name || robloxId} (ID: ${robloxId})\n**Новый ранг:** ${rankName}\n**Выдал:** ${interaction.user.tag}`).setTimestamp()); }
});
